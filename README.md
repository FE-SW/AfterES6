...
### 정적 vs. 동적 로딩:
* ES Modules (ESM): import와 export 문은 정적으로 분석된다. 즉, 모듈 의존성은 코드가 실행되기 전에 컴파일 단계에서 결정된다. 이로 인해 트리 쉐이킹과 같은 최적화가 가능하다.모듈 의존성이 명확하게 정의되어 있으며, 코드가 실행되기 전에 모든 의존성이 해결된다.
* CommonJS: require() 함수를 통해 동적으로 모듈을 로드할 수 있다. 이를 통해 런타임에 모듈 의존성을 변경할 수 있다. 모듈을 필요할 때 로드할 수 있으며, 조건에 따라 다른 모듈을 로드할 수 있다.
...
## 동기여부
* ES Modules (ESM): 브라우저 환경에서 import는 비동기적으로 모듈을 로드할 수 있다. 이는 네트워크 요청이 비동기적으로 처리되기 때문에 브라우저에서 더 효율적이다.모듈 로딩이 비동기적으로 이루어지며, 코드 실행이 차단되지 않는다.
* CommonJS: require()는 동기적으로 모듈을 로드합니다. 즉, 모듈이 로드될 때까지 코드 실행이 멈춥니다. 이는 서버 환경에서는 문제가 되지 않지만, 브라우저 환경에서는 비효율적일 수 있다. 모듈이 로드될 때까지 코드 실행이 차단된다.
...
| 모듈 내보내기 | module.exports 또는 exports | export 또는 export default |
| 로딩 방식 | 동기적 로딩 | 비동기적 로딩 |
| 정적 분석 가능성 | 불가능 | 가능 (트리 쉐이킹 지원) |
| 사용 환경 | 주로 Node.js | 브라우저와 Node.js |
| 실행 시점 | 런타임 시 로드 | 정적 분석을 통해 컴파일 시 로드 가능 | 
